...
def intermediate_dimreduce(coords):
    ...
    Comment: Choice / detail of this function should depend on the system of interest. Since
    ...
    This function calculates pairwise distances between atoms in the backbone of a molecular structure
    for each frame in the provided coordinates, comparing them to a reference structure.
...
def haMSM_result_aggregated(
    ...
    # Read in flux matrix 
    with open(haMSM_path+'/flux_matrices.pkl', 'rb') as file:
      # Load the data from the pickle file
        flux_matrices = pickle.load(file)
    
    average_flux_matrix = np.mean(flux_matrices, axis=0)
    
    # Cluster center info
    # They should all be the same
    cluster_center = np.loadtxt(haMSM_path+'/cluster_centers.txt')
